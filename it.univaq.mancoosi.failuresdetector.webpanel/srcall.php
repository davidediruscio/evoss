...
<?php
$campis = SQL('SELECT * FROM '.$_GET['t'].' ORDER BY ID ASC');
if ($campis != null){ 
print "<table style='width: 100%;' align='center'>";
$cont = 0;
foreach($campis as $k => $campix) { 
while ($cont < $numerorighe){
if ($cont == 0){ print "<tr><td style='width: 100%; border: 0px; margin: 0px;'><form id='".$_GET['t']."_".$campix[$campi[$cont]]."' width='100%'><table style='width: 100%; margin: 0px;' id='".$_GET['t']."_".$campix[$campi[$cont]]."'><tr><td class='art-postmetadataheader' style='width: 100%; text-align: center;' colspan='2'><b> RECORD ".strtoupper($campi[$cont]).": ".$campix[$campi[$cont]]."</b> &nbsp; &nbsp; <a href='javascript:deletethis(\"".$_GET['t']."\",".$campix[$campi[$cont]].");'><img src='images/cancella.gif' width='10' style='border: 0px; margin: 0px;' /></a></td></tr>"; $IDz = $campix[$campi[$cont]]; }
else { 
